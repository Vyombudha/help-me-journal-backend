...
import { newContainerInput, updateContainerSchema, ContainerIdParam, UpdateContainerInput, } from "./containers.schema.js";
...
const newContainer = async (
    ...
    const newContainer = await ContainerService.new(userId, projectId, body.title, body.type, body.moods);
    res.status(200).json({
        success: true,
        data: newContainer
    });
