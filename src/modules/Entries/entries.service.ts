...
const createEntry = async (ownerId: string, containerId: string, title: string, content: string): Promise<Entry> => {
...
const getAllEntries = async (ownerId: string, containerId: string): Promise<Entry[]> => {
    await prisma.container.findFirstOrThrow({
        where: {
            id: containerId,
            project: {
...
const updateEntryData = async (ownerId: string, entryId: string, newTitle: string, newContent: string): Promise<Entry> => {
